...
const defu = createDefu((obj, key, value) => {
  if (key === 'rules' && typeof value === 'object' && !Array.isArray(value)) {
    const rules = obj[key]
    if (!rules) {
      obj[key] = {} as typeof obj[typeof key]
    }
    for (const ruleName in value) {
      (obj[key] as Record<string, unknown>)[ruleName] = value[ruleName]
    }
...
async function config(options?: AntfuOptions) {
  const defaultOptions: AntfuOptions = {
    rules: {
  ...
  const mergedOptions = options
    ? defu<AntfuOptions, [AntfuOptions]>(options, defaultOptions)
    : defaultOptions

  const config = await antfu(mergedOptions)
